...
// 来源项目列表（支付宝系）
const getSource = (value, modules = []) => {
	const { source, appId } = value;
	if (source != 'alipay') {
		return [];
	}
	return modules.filter(item => item.appId == appId).map(item => item.platform);
}
...
const collect = async (modules) => {
	...
	const sourceList = getSource(find, modules);
	action(find, {
		...answer,
		sourceList,
	});
